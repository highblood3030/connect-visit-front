...
export default function Layout({ children }) {
  ...
  return (
    <div 
      className="min-h-screen bg-gradient-to-br from-[#FAE7C9] to-[#b4f6ff] flex relative"
      onClick={handleOutsideClick}
    >
      {/* Navbar */}
      <nav className="bg-[#91C8C4] text-white flex items-center h-16 px-6 shadow-md fixed top-0 left-0 w-full z-50">
        <div className="ml-auto flex items-center">
          <img src="/QR-Logo.png" alt="QR-Logo Logo" className="w-16 h-12 object-contain shadow-md" />
        </div>
      </nav>

      {/* Mini Sidebar - Always Visible */}
      <aside 
        className="w-16 h-screen bg-[#D7F0ED] text-[#145C5B] flex flex-col items-center py-6 space-y-6 border-r border-gray-300 shadow-xl fixed top-0 left-0 z-50"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Hamburger Menu */}
        <button 
          onClick={() => setSidebarOpen(true)} 
          className="p-3 rounded-full bg-white shadow-lg hover:bg-gray-200 transition-transform focus:outline-none mb-6"
        >
        ...
        {sidebarItems.map((item, idx) => (
          <div key={idx} className="relative">
            ...
            >
              <item.icon className="text-3xl font-bold" />
            </div>

            {/* Mini Sidebar Dropdown for Conneq Biz */}
      ...
      {/* Floating Sidebar */}
      <div className={`fixed top-0 left-0 h-screen w-72 bg-[#D7F0ED] shadow-xl transition-transform duration-500 z-50 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`} onClick={(e) => e.stopPropagation()}>
        <div className="flex flex-col items-center p-6">
          <img src="/profile-placeholder.png" alt="Profile" className="w-24 h-24 rounded-full border-4 border-[#145C5B] shadow-md" />
        ...
        <div className="mt-4 space-y-1 text-gray-700">
          {sidebarItems.map((item, idx) => (
            <div key={idx} className="relative">
              ...
              {/* Floating Sidebar Dropdown for Conneq Biz */}
              {item.dropdown && bizDropdownOpen && (
                <div className="pl-10 space-y-1">
                  {item.dropdown.map((subItem, subIdx) => (
                    <div
                      key={subIdx}
                      onClick={() => { router.push(subItem.path); setSidebarOpen(false); }}
                      className="cursor-pointer px-3 py-2 hover:bg-gray-200 rounded"
                    >
                      {subItem.label}
                    </div>
        ...
        <div className="absolute bottom-4 left-4">
          <button onClick={() => router.push("/logout")} className="flex items-center space-x-3 px-4 py-3 cursor-pointer text-red-600 hover:bg-red-100 rounded-lg transition">
      ...
      {/* Main Content */}
      <main className="flex-grow p-8 ml-16">{children}</main>
    </div>
  );
}
